...
void GUI::renderAntiAimWindow(bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("Ligado?", &config->antiAim.enabled);
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("Ativa o Anti Aim e libera configurações");
    }
    
        ImGui::Checkbox("Angulo X?", &config->antiAim.pitch);
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("Altera o ângulo vertical do personagem");
        }
        if (config->antiAim.pitch) {
            ImGui::SameLine();
            ImGui::SetNextItemWidth(240.0f);
            ImGui::SliderFloat("X", &config->antiAim.pitchAngle, -89.0f, 89.0f, "%.2f", 1);
        }
        ImGui::Checkbox("Angulo Y?", &config->antiAim.yaw);
        config->misc.fixMovement = config->antiAim.yaw;
        
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("Altera o ângulo horizontal do personagem");
        }
        if (config->antiAim.yaw) {
            ImGui::SameLine();
            ImGui::SetNextItemWidth(240.0f);
            ImGui::SliderFloat("Y", &config->antiAim.yawAngle, -179.0f, 179.0f, "%.2f", 1);
            ImGui::Text("Inverter na tecla");
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Essa tecla inverterá o Ângulo Y do AntiAim");
            }
            ImGui::SameLine();
            ...
            ImGui::Combo("", &config->antiAim.yawInverseKeyMode, "Segurar\0Ativar\0");
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Segurar = Segure para ativar | Ativar = Aperte para ativar, aperte de novo para desativar");
            }
            ImGui::PopID();
        ...
        else
        {
            ImGui::Text("Inverter na Tecla");
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Essa tecla inverterá o do AntiAim");
            }
            ImGui::SameLine();
            hotkey(config->antiAim.yawInverseAngleKey);
        }
        ImGui::Checkbox("Desync Y", &config->antiAim.yawReal);
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("Dessincroniza o Ângulo Y");
        }
        if (config->antiAim.yawReal == true)
        {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Essa função ativa o Body Lean");
            }
            ImGui::Checkbox("LBY Breaker", &config->antiAim.LBYBreaker);    
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Essa função ativa o LBY Breaker");
            }

            if (config->antiAim.LBYBreaker)
            {
                ImGui::SliderFloat("Angulo LBY", &config->antiAim.LBYAngle, 0.0f, 180.0f, "%.2f", 1);
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip("Ângulo do LBY Breaker");
                }
            }
        }
        ImGui::SetNextItemWidth(85.0f);
        ImGui::Combo("Modo do Anti-Aim", &config->antiAim.mode, "Estatico\0Custom\0");
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("Estático = AntiAim Parado | Custom = AntiAim mudando");
        }
        if (config->antiAim.mode == 1)
        {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("O Jitter Máximo do AntiAim");
            }
            ImGui::SetNextItemWidth(240.0f);
            ImGui::SliderFloat("Jitter Min", &config->antiAim.jitterMin, -179.0f, 179.0f, "%.2f", 1);
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("O Jitter Mínimo do AntiAim");
            }
        }
...
void GUI::renderChamsWindow(bool contentOnly) noexcept
{
    ...
    if (ImGui::Combo("", &currentCategory, "Aliados\0Inimigos\0Plantando\0Defusando\0Você\0Armas\0Maos\0Backtrack\0Sleeves\0"))
        material = 1;
    if (ImGui::IsItemHovered()) {
...
void GUI::renderVisualsWindow(bool contentOnly) noexcept
{
    ...
    ImGui::SetNextItemWidth(75.0f);
    ImGui::Combo("Âng. 3ª Pessoa", &config->antiAim.thirdpersonMode, "Falso\0Real\0Tick Atual\0");
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("Exibe os ângulos do AntiAim/FakeLag");
    }
